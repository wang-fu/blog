...
type: post
blog: true
description: 全书第一章，建立坐标系：AI 数字员工与聊天机器人、自动化脚本的本质差别在哪，为什么「判断归 AI，执行归脚本」是贯穿全书的设计原则。
tags:
    - AI Agent
    - 数字员工
    - 开源书籍
...
我曾经发过一段这样的视频：让 Agent 去京东买一瓶薰衣草香精，”100 ml、法国产、评价 4.8 以上”。屏幕上，浏览器自主弹出，搜索框被敲入关键词，结果一条条滚过；它在第三条停下，点进详情页，比对产地、容量、用户评价；返回列表，再点开另一条做二次对比；筛选条件被调整一次又一次，购物车里的商品被加入又被清空，最后定格在结算页。从输入到结算，全程没有人碰键盘鼠标——单看演示，这条路看起来已经通了。
...
### 1.4 系统全景：一个 AI 数字员工系统需要什么

### 问题：从原型到系统的距离

前三节讲了”AI 数字员工是什么”——AI 做判断，脚本做执行，两层市场互驱。如果你被说服了，想自己造一个，第一个问题是：需要建多少东西？

做一个”能对话的 AI 助手”原型很快。调一个 LLM API，把用户输入发过去，把回复显示出来。一个周末就能跑起来。

但”能对话”距离”能当员工”差了整整一个系统的距离。

员工要有工作时间——谁在什么时候叫醒他？需要一个调度引擎。员工要记得昨天做过什么——跨次运行的状态存在哪？需要一个记忆系统。员工要能操作企业系统——怎么登录、怎么点按钮？需要一个浏览器环境。员工做完事要汇报——通知谁、用什么通道？需要一个通信体系。员工可能同时服务多个用户、多个任务——资源怎么分配、冲突怎么处理？需要并发控制。

这些东西单独拿出来都不难理解，但它们加在一起构成了一个完整系统的骨架。理解这个骨架，是读后面每一章的前提——否则你知道每块砖长什么样，但不知道它砌在哪面墙上。

### 五层：从上到下

把这些需求按职责分层，大致会落成五个层次：

**用户交互层。** 用户看到什么、怎么操作。对话界面、文件管理、数字员工的状态面板。这一层不关心 AI 怎么跑，只关心怎么展示结果和接收指令。

**数字员工编排层。** 一个数字员工”长什么样”（定义）、”什么时候醒”（调度）、”这次做了什么”（运行记录）。这一层回答的是”谁在什么时候做什么事”。

**服务层。** 具体能力的实现。Agent 对话、AI 浏览器操作、多供应商模型接入、远程访问、健康监控、通知推送。编排层说”现在该用浏览器查一下 OA”，服务层负责”怎么打开浏览器、怎么点那个按钮”。

**平台基础设施层。** 不含业务语义的通用原语——存储、调度、事件总线、记忆、后台保活。这一层不知道”数字员工”是什么，就像操作系统不知道你装了什么应用。任何上层模块都可以用它，它不依赖任何上层。

**AI 引擎层。** Agent Loop 的核心循环——调用 LLM、解析工具调用、执行工具、把结果塞回上下文、再调 LLM。这是整个系统的推理心脏。

依赖方向只能向下。UI 可以调编排层，编排层可以调服务层，服务层可以调平台层和引擎层。反过来不行——平台层不知道服务层的存在，服务层不知道编排层的存在。

但数据流不是单向的——**调用向下，事件向上**。调度器（平台层）到了 cron 时间，通过运行时（编排层）启动一次执行；执行过程中 Agent Loop（引擎层）调用浏览器工具（服务层），拿到结果后写入运行记录（编排层），前端（UI 层）通过事件订阅实时渲染。向下是请求，向上是推送。这条双向管道贯穿全书。

### 为什么是这五层而不是别的

分层不是为了画图好看。每一层的存在都对应一个具体的隔离需求：

**UI 和后端分离**是因为 AI 数字员工可能在后台无人值守地运行——根本没有 UI。如果业务逻辑和 UI 耦合在一起，无界面运行就做不了。

**编排层和服务层分离**是因为”什么时候做”和”怎么做”的变化节奏不同。用户改了数字员工的工作规则（编排层），不应该影响浏览器怎么打开网页（服务层）。反过来，浏览器工具升级了，不应该影响数字员工的调度逻辑。

**平台层独立**是因为存储、调度、事件这些原语是所有业务的共同需求。如果调度器代码里写死了”数字员工”的概念，以后想给对话系统也加定时功能，就得复制一份调度代码。平台层不含业务语义，所以它可以被任何业务复用。

**AI 引擎独立**是因为模型和协议在快速迭代。今天用 Claude，明天可能加 GPT，后天可能换一个开源模型。引擎层封装了协议差异，上层不需要知道底下跑的是哪个模型。

这套分层逻辑不是 Halo 独有的。VS Code 的源码也是类似结构——`workbench`→`platform`→`base`，单向依赖，几百个 contributor 靠这个纪律共存。任何超过一定规模的系统，最终都会走向这种分层——因为不分层的代价（改一处崩十处）会随着代码量指数增长。

### 对话和自动化共享同一条管道

这五层架构有一个设计后果值得提前说明：**对话和数字员工走的是同一条管道。**

用户在界面输入”帮我查一下今天的待办”，消息从 UI 层进入服务层的 Agent 模块，Agent 模块启动 Agent Loop，Loop 调用工具、拿到结果、返回给用户。

数字员工的路径完全一样——只是入口不是用户手动发送，而是调度器到了 cron 时间自动触发。从 Agent Loop 往下，对话和自动化共享同一个引擎、同一套工具、同一个执行环境。

这意味着数字员工的能力上限不局限于审批、监控这类简单任务——写代码、生成报告、操作浏览器完成复杂调研，任何 AI Agent 能在对话中完成的事，数字员工都可以自主完成。第三章会展开这个引擎的完整架构。

### 本地优先：数据在你的机器上

这套系统的数据架构做了一个有意识的选择：所有数据都在本地。

没有云数据库，没有账号系统，断网照常运行。API Key 存在本地配置文件里，对话记录存在本地工作空间目录里，数字员工的运行日志存在本地 SQLite 里。

这个选择有明确的产品理由：如果你的数据存在别人的服务器上，你的 AI 就受制于别人的定价和规则。本地优先意味着用户完全控制自己的数据和运行环境。

代价同样明确：多设备同步需要额外机制。Halo 的解法是远程访问——通过隧道把桌面端暴露到网络，从手机或浏览器连进来。数据始终在一台机器上，访问可以在任何地方。第十一章展开。
...
| --- | --- | --- |
| 第二章 | 全局 | 三端统一的应用骨架怎么搭 |
| 第三章 | AI 引擎 | Agent Loop 怎么让 AI 可靠地完成工作 |
| 第四章 | AI 引擎 | 多家模型的协议差异怎么消化 |
| 第五章 | 编排层 | 一个数字员工的完整定义是什么 |
| 第六章 | 平台+编排 | 调度引擎怎么可靠地按时唤醒 |
| 第七章 | 平台 | 跨次运行的记忆怎么设计 |
| 第八章 | 服务层 | AI 浏览器怎么构建 |
| 第九章 | 服务层 | 数字员工怎么主动联系人 |
| 第十章 | UI 层 | 流式渲染和交互工程 |
| 第十一章 | 服务层 | 远程访问和稳定性保障 |
| 第十二章 | 平台 | Skill 和数字员工怎么分发 |

知道你读的技术处于哪一层、解决的是什么设计问题，就不会迷路。

---

### 1.5 零适配接入企业内网：复用浏览器登录态

### 问题：对接一个内网系统要多久

任何成规模的企业里都有几十套系统——OA、工单、知识库、发布平台、监控面板。每套系统都是 Web 页面，每套都有自己的接口格式。

想让 AI 自动化操作其中一个系统，传统做法是什么？找那个系统的后端团队，要 API 文档，排期联调。一套系统对接下来，几周算快的。企业里有几十套系统，要做几十次。MCP 协议火了之后换个说法——”给每套系统写个 MCP Server”——本质没变，还是一个一个对接。

这条路的成本之所以高，是因为它要求**目标系统配合**——开接口、给文档、联调。而内网系统的后端团队通常有自己的排期，你的自动化需求在他们那里排不上优先级。

但退一步想：你的员工每天是怎么用这些系统的？打开浏览器，登录，点按钮。浏览器里存着 cookie，系统认的是 cookie，不是人。

### 设计思想：不对接系统，复用浏览器

如果 AI 也在同一个浏览器里操作呢？

cookie 在那里，网络请求从用户自己的电脑发出去，内网系统看到的和员工自己操作完全一样。不需要开 API，不需要写适配器，不需要 IT 部门配合。

这个思路的核心是**把浏览器的登录态当作 AI 进入企业系统的通道**。用户在浏览器里手动登录一次，cookie 存下来，之后 AI 的所有操作都复用这个身份。内网系统不知道请求来自 AI——它看到的是一个正常的、有权限的、从员工电脑发出的 HTTP 请求。

权限边界天然清晰：AI 用的是你的账号，做的是你有权限做的事。不可能越权，因为 cookie 代表的就是你自己。

### 这和 RPA 有什么不同

RPA 也是在 UI 层操作，但它依赖固定的元素定位——CSS 选择器、XPath、屏幕坐标。界面改版，脚本就崩。

浏览器登录态复用走的是另一条路：不操作 UI 元素，而是直接调用前端页面背后的 HTTP 接口。任何 Web 系统的前端页面都在调接口——打开 F12 就能看到。Skill 做的事就是复用这些接口，带着浏览器里已有的 cookie 发请求。

接口比 UI 元素稳定得多。按钮位置挪了、页面样式改了，接口通常不变。接口变更的频率远低于 UI 变更——这就是为什么这种方式比 RPA 脚本更健壮。

当然，接口也可能变。内网系统升级改了接口路径或参数格式，Skill 跟着要改。但这个维护成本和”从零对接 API”不在一个量级——你改的是一个 `fetch` 调用的 URL，不是重新走一遍联调流程。

### 两个需要解决的工程问题

思路很简单，但落地有两个工程问题。

**第一：Electron 不是真正的 Chrome。** 带反爬虫机制的站点会检测浏览器指纹——`navigator.webdriver` 被设成 `true`、`window.chrome` 对象缺字段、WebGL 渲染器字符串不对。这些细节暴露了自动化工具的身份。解决方案是注入一组反检测脚本，在页面 JavaScript 执行之前修正这些指纹，让 Electron 在目标系统面前等同真实 Chrome。思路来自 puppeteer 社区的 stealth 插件，不是原创，但集成进 Electron 桌面应用并对用户透明，这个工程整合是必须做的。第八章展开实现。

**第二：AI 怎么理解页面内容。** 截图是最直觉的方案——但一张截图几千 Token，十几步操作下来成本积累很快，而且视觉定位存在坐标偏差。更高效的方案是利用浏览器的无障碍树（Accessibility Tree）——本来是给屏幕阅读器用的，描述页面上每个可交互元素的角色、名称、状态。AI 看到的是结构化文本（”button ‘审批’“、”textbox ‘备注’“），不是像素。Token 消耗小一个数量级，定位精度也更高。第八章展开完整实现。

但需要强调：无障碍树和截图都是**探索和兜底手段**。日常跑业务的主路径是 Skill——一段写好的脚本直接调接口，AI 不需要”看”页面，也不需要”点”按钮。只有在目标系统还没人写过 Skill、需要 AI 从零摸索流程时，才用无障碍树引导操作。

### 边界

“零适配”有明确的适用条件，不是万能的。

**要先手动登录一次。** 用户在内置浏览器里手动登录目标系统，cookie 才会存下来。session 过期了，需要重新登。这和用真浏览器一样——没有什么系统能永远保持登录态。

**只能做你有权限做的事。** AI 用的是你的账号。你看不到的页面、你点不了的按钮，AI 也做不到。这既是限制，也是安全保障——不存在越权的可能。

**依赖前端接口的稳定性。** Skill 调的是前端页面使用的接口，没有版本保证。但如前所述，接口变更频率远低于 UI 变更，维护成本可控。

**需要 Electron 才能实现。** 这个方案依赖内置 Chromium 提供的完整浏览器环境——cookie 隔离、CDP 协议、脚本注入。纯 Web 应用做不到本地浏览器的 session 管理，原生 GUI 框架做不到内嵌 Chromium。这也解释了为什么 Halo 选了 Electron——不是因为它轻量（它很臃肿），是因为它提供了打开企业内网的那扇门。

### 从三个月到三分钟

传统企业内网对接：拉后端同事开会→要 API 文档→排期→联调→上灰度。一套系统几周，几十套系统几个月。

浏览器登录态复用：登录一次，写一个 Skill 调前端接口。门槛从”需要 IT 部门配合”降到”会用浏览器开发者工具就行”。

核心就一句话：**你的浏览器能登录什么，AI 就能操作什么。**
...
三代方案的演进给出了第一个结论：AI 擅长判断，不擅长可靠地重复执行。把”做不做”和”怎么做”拆成两个独立决策，整个系统的可靠性、成本、可复用性同时解锁。Skill 做执行，数字员工做判断，两层通过文件系统解耦。

Skill 市场给 AI 装能力，数字员工市场把这些能力组织成可调度的工作单元。两层互相驱动，缺一不可：只有 Skill 而没有数字员工，非技术用户用不起来；只有数字员工而没有 Skill，能力无法跨场景积累。

一个完整的 AI 数字员工系统需要五层——UI、编排、服务、平台、引擎——依赖只能向下。这种分层不是 Halo 的发明，是任何超过一定规模的系统最终都会走向的结构，因为不分层的代价会随代码量指数增长。对话和数字员工共享同一条管道，意味着数字员工的能力上限等于 Agent 本身的能力上限。

零适配接入是这套系统最直接的落地方式。不对接系统的后端 API，复用用户浏览器的登录态，让 AI 通过 cookie 进入企业内网。你能登录的系统，AI 就能操作。代价是需要 Electron 提供完整的 Chromium 环境，以及 session 过期时需要重新登录。

这五件事构成后面所有技术章节的坐标系。读每一章时，可以对照定位：这个模块属于哪一层，处理的是判断还是执行，解决的是什么设计问题。

下一章拆开工程地基——运行时 Adapter、声明式能力边界、分层与依赖方向、性能预算、命名空间迁移。这些不是 AI 相关的技术，但没有它们，后面所有 AI 能力都无处安放。

---

> 本文是《如何从零构建 7×24 小时 AI Agent》的第 1 章。
>
> - 全书在线阅读：<https://book.imwangfu.com/>
> - 全书仓库与反馈：<https://github.com/openkursar-flynn/build-ai-agent-platform>
> - 书中所述系统的开源实现 Halo：<https://github.com/openkursar/hello-halo>
