---
title: 类、hook、组合式 api 中关于前端复用的思考和几点建议
...
复用，在 vue 前端层面有多种形式：指令、filters(vue3 废弃)、minx(vue 3 废弃)、hook，计算属性等。
...
鉴于官方文档并没有深刻、详细介绍最佳实践，且例子对初学者隐藏了很多设计背后的东西，导致被不少同学奉这种新 api 为圣经且无法正确的运用。在这里深入浅出的剖析达下背后设计理念。
...
react 团队难以忍受高阶组件带来的深层嵌套，创造了一种新 DSL，叫 hook，发布在 react 16 版本中，随后很快流行起来。vue 3 以组合式 api 的形式提供了类似的思想，以下我们统称 hook。
...
这个函数内部同时带有状态和行为，行为等同于对象中的方法，状态则是对象中上下数据。数据被修改后，最后函数的值重新被返回给调用方。
...
这确实是一个精妙的设计，最早想到这个 idea 的人是个鬼才，软化行业没有什么问题不能够多加一层解决，把面向对象的横切面复用问题，通过 DSL 包装到普通函数中去，让复用模块能像函数意义被方便的调用。
...
一个组件，如果向上复用了更多的状态，只要中间任意一个状态被意外修改，组件产生的疑难杂症非常难定为，这和多继承的隐患相似。尽量在架构设计层面规避这种场景发生。
...
封装大规模状态和行为首先不符合我们函数的最佳实践。其次，前端组件设计层面，我们从来不推荐有大组件、更不会有大的被复用模块，在 react、vue 均提倡拆分，组合的思维，遇到这种场景，只能说你拆分设计不够细。相反，如果你的应用规模对象足够多且复杂，使用类似 angular，用完全的面向对象思维和语法去构建更合适。但常见的前端页面并不具备这种大规模面相对象的特点。
...
我们看上面第一个，getData 只是一个获取数据的异步方法而已，hook 会响应式自动更新 someData 数据。而人的思维天生对同步编程方式理解成本更低。响应式只应该出现在人不该关心的地方，人关心的地方，应该是让开发者手动去调用，否则，大脑还要分析消息派发的来源。这种只是小例子，大部分情况下不要为了少写一行代码，而增加理解成本，复用的目的从来不是少码字，而是为了提高可读、易拓展。


我提到过，只要是状态，不管通过类，还是 hook 等方式，比纯函数危险都更高。而 99% 的场景可以设计成不需要状态共用，让横切关注点降低。而跨组件共享的状态，大部分又能通过专门的状态管理解，但 redux 状态管理的作者也说过 99% 的前端场景不需要使用状态管理。

所以，hook 请不要滥用，记住上面的最佳实践和使用场景。
