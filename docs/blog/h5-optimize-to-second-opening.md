...
type: post
blog: true
excerpt: 在传统的 web 优化中，我们可以通过减小、拆包等减少资源大小，html 页面直出等方法达到页面打开速度控制在 1s 以内，可以参考之前一篇分析[h5 秒开大全]（）。在大部分场合中，这些方案都足够用，也能得到出色的效果。但这些方案有两种无法尽善尽美的地方：其一是加载白屏现象不可避免，其二是对于超大型 web 应用效果有限。结合客户端特性，我们有没有办法，进一步做到极致，做到打开 web 页面和客户端页面无差异的体验呢？
tags:
    - web
    - 优化
    - 性能
